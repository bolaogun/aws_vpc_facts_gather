...
def main():
    ...
    # Doing this because boto appear NOT to know about the class 
    # boto.vpc.vpnconnection.VpnConnectionOptions
    vpn_id = vpn_conn.get("id", "None") 
    cust_gw_config = vpn_conn.get("customer_gateway_configuration", "None")
    vpn_gw_id = vpn_conn.get("vpn_gateway_id", "None")
    vpn_state = vpn_conn.get("state", "None")
    vpn_typ = vpn_conn.get("type", "None")
    vpn_cust_gw_id  = vpn_conn.get("customer_gateway_id", "None")
    vpn_tnnls =  vpn_conn.get("tunnels", "None")
    vpn_opts =  vpn_conn.get("options", "None")
    vpn_sts_rts = vpn_conn.get("static_routes", "None")
    vpn_status = vpn_conn.get("status", "None")

    ec2_facts_result = { "changed": vpn_conn_man.changed,
                         "vpn_id": vpn_id,
                         "state": vpn_state,
                         "type": vpn_typ,
                         "customer_gateway_id": vpn_cust_gw_id,
                         "tunnels": vpn_tnnls,
#                         "options": vpn_opts,
                         "static_routes": vpn_sts_rts,
                         "status": vpn_status,
                         "vpn_gateway_id": vpn_gw_id,
                         "customer_gateway_configuration": cust_gw_config,
